...
def _valid_agent_text() -> str:
    return textwrap.dedent("""\
        ---
        name: agente_profundidad_colaboracion
...
def _valid_orchestrator_text() -> str:
    return textwrap.dedent("""\
        # agente_orquestador_pipeline

        At every FULL checkpoint and at pipeline completion, dispatch
        agente_profundidad_colaboracion (advisory only; never blocks).
        """)
...
def _valid_skill_md_text() -> str:
    return textwrap.dedent("""\
        ...
        The agente_profundidad_colaboracion runs at checkpoints as advisory only and
...
def _make_repo(root: Path) -> None:
    ...
    agents = root / "pipeline-academico" / "agents"
    agents.mkdir(parents=True)
    (agents / "agente_profundidad_colaboracion.md").write_text(_valid_agent_text())
    (agents / "agente_orquestador_pipeline.md").write_text(_valid_orchestrator_text())
    (root / "pipeline-academico" / "HABILIDAD.md").write_text(_valid_skill_md_text())
...
class TestCollaborationDepthRubric(unittest.TestCase):
    ...
    def test_agent_without_rubric_ref_fails(self) -> None:
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            _make_repo(root)
            agent = root / "pipeline-academico" / "agents" / "agente_profundidad_colaboracion.md"
    ...
    def test_orchestrator_missing_invocation_fails(self) -> None:
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            _make_repo(root)
            orch = root / "pipeline-academico" / "agents" / "agente_orquestador_pipeline.md"
            orch.write_text("# agente_orquestador_pipeline\n\nNo observer mentioned.\n")
            r = _run(root)
            self.assertEqual(r.returncode, 1)
            self.assertIn("agente_profundidad_colaboracion", r.stdout)
    ...
    def test_orchestrator_without_completion_context_fails(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            orch = root / "pipeline-academico" / "agents" / "agente_orquestador_pipeline.md"
            orch.write_text(
                "At FULL checkpoint, dispatch agente_profundidad_colaboracion.\n"
            )
            r = _run(root)
            self.assertEqual(r.returncode, 1)
    ...
    def test_orchestrator_keyword_only_without_dispatch_fails(self) -> None:
        ...
        with TemporaryDirectory() as tmp:
            ...
            orch.write_text(
                ...
                "The Agent Team includes agente_profundidad_colaboracion as the "
    ...
    def test_skill_md_without_nonblocking_phrase_fails(self) -> None:
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            _make_repo(root)
            skill = root / "pipeline-academico" / "HABILIDAD.md"
            skill.write_text("# pipeline-academico\n\nWe use agente_profundidad_colaboracion.\n")
            r = _run(root)
            self.assertEqual(r.returncode, 1)
