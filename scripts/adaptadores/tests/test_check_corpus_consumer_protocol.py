...
def fixture_repo(tmp_path: Path) -> Path:
    ...
    manifest = {
        "supported_consumers": [
            {
                "agent_path": "investigacion-profunda/agentes/agente_bibliografia.md",
                "agent_basename": "agente_bibliografia",
    ...
    ref_dir = tmp_path / "pipeline-academico" / "references"
    ...
    # --- agente_bibliografia.md with all required prose markers
    agent_dir = tmp_path / "investigacion-profunda" / "agents"
    agent_dir.mkdir(parents=True)
    (agent_dir / "agente_bibliografia.md").write_text(
        textwrap.dedent(
            """\
            ---
            ...
            See `pipeline-academico/referencias/literature_corpus_consumers.md`.
...
def test_l3_fails_when_backpointer_missing(fixture_repo: Path) -> None:
    agent = fixture_repo / "investigacion-profunda" / "agents" / "agente_bibliografia.md"
    text = agent.read_text().replace(
        "pipeline-academico/referencias/literature_corpus_consumers.md", ""
    )
...
def test_l8_passes_pr_a_with_caveat_remaining(fixture_repo: Path) -> None:
    """PR-A state: manifest = {agente_bibliografia}, caveat MUST remain."""
    result = run_lint(fixture_repo)
    assert result.returncode == 0, result.stdout + result.stderr
...
def test_l8_passes_pr_b_with_caveat_retired_and_backpointer(fixture_repo: Path) -> None:
    ...
    manifest["supported_consumers"].append(
        {
            "agent_path": "articulo-academico/agentes/agente_estratega_literatura.md",
            "agent_basename": "agente_estratega_literatura",
    ...
    ref = fixture_repo / "pipeline-academico" / "references" / "literature_corpus_consumers.md"
    text = ref.read_text()
    ...
    # Create the second consumer agent file by cloning agente_bibliografia.md content
    strat_dir = fixture_repo / "articulo-academico" / "agents"
    strat_dir.mkdir(parents=True)
    biblio_text = (
        fixture_repo / "investigacion-profunda" / "agents" / "agente_bibliografia.md"
    ).read_text()
    (strat_dir / "agente_estratega_literatura.md").write_text(
        biblio_text.replace(
            "name: agente_bibliografia", "name: agente_estratega_literatura"
        )
    )
...
def test_l8_fails_invalid_third_state_strategist_only(fixture_repo: Path) -> None:
    """Manifest with only agente_estratega_literatura must fail L8."""
    manifest = {
        "supported_consumers": [
            {
                "agent_path": "articulo-academico/agentes/agente_estratega_literatura.md",
...
def test_l8_fails_duplicate_basename_with_different_path(fixture_repo: Path) -> None:
    ...
    basenames only. A future manifest could stash a second agente_bibliografia
    ...
    manifest["supported_consumers"].append(
        {
            "agent_path": "fake/agentes/shadow_agente_bibliografia.md",
            "agent_basename": "agente_bibliografia",
            "skill": "articulo-academico",
            "since_version": "v3.6.5",
            "phase": "Phase 1",
...
def test_l8_fails_when_path_diverges_from_canonical(fixture_repo: Path) -> None:
    ...
    manifest = {
        "supported_consumers": [
            {
                "agent_path": "investigacion-profunda/agentes/MOVED_agente_bibliografia.md",
                "agent_basename": "agente_bibliografia",
                "skill": "investigacion-profunda",
                "since_version": "v3.6.5",
                "phase": "Phase 1",
...
def test_l8_fails_invalid_third_state_extra_unknown(fixture_repo: Path) -> None:
    ...
    manifest["supported_consumers"].append(
        {
            "agent_path": "fake/agentes/agente_cumplimiento_citas.md",
            "agent_basename": "agente_cumplimiento_citas",
            "skill": "articulo-academico",
            "since_version": "v3.6.5",
            "phase": "Phase 5a",
        }
