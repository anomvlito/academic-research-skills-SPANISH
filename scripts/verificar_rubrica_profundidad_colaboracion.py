...
ORCHESTRATOR_PATH = Path("pipeline-academico/agentes/agente_orquestador_pipeline.md")
PIPELINE_SKILL_PATH = Path("pipeline-academico/HABILIDAD.md")
AGENTS_DIR = Path("pipeline-academico/agents")
...
DISPATCH_RE = re.compile(
    rf"(?is)\b(?:{_VERBS})\b.{{0,80}}?\bagente_profundidad_colaboracion\b"
    rf"|\bagente_profundidad_colaboracion\b.{{0,80}}?\b(?:{_VERBS})\b"
)
...
def _check_orchestrator(root: Path) -> list[str]:
    ...
    text = path.read_text(encoding="utf-8")
    if "agente_profundidad_colaboracion" not in text:
        return [f"{ORCHESTRATOR_PATH}: does not mention 'agente_profundidad_colaboracion'"]
    ...
    if not DISPATCH_RE.search(text):
        return [
            f"{ORCHESTRATOR_PATH}: mentions agente_profundidad_colaboracion but "
            f"no dispatch anchor (e.g. 'dispatch agente_profundidad_colaboracion', "
            f"'invoke agente_profundidad_colaboracion')"
        ]
...
def _check_skill_md(root: Path) -> list[str]:
    ...
    if "agente_profundidad_colaboracion" not in text:
        return [f"{PIPELINE_SKILL_PATH}: does not mention agente_profundidad_colaboracion"]
