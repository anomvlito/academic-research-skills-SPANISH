...
Este documento lists all failure scenarios that may be encountered across all modes of the investigacion-profunda skill, along with their detection conditions, user notification messages, handling steps, and recovery paths. The purpose is to ensure every failure scenario has a clear handling strategy, preventing users from reaching a dead end.
...
|---|---------|---------|---------|---------|
| F1 | PI cannot converge | full, socratic | Medium | Narrow scope / provide candidate RQs |
...
| F4 | Abogado del Diablo CRITICAL | full | Critical | STOP + correct |
...
### F1: Pregunta de Investigación Cannot Converge
...
> I notice we've been discussing for a while, but the Pregunta de Investigación hasn't converged to a clear direction yet. This is perfectly normal — sometimes the question itself is the hardest part. Let me offer a few possible directions to see which one is closest to your thinking.
...
**Recovery Paths**:
- Select a candidate PI → continue the original workflow
- Do lit-review → restart PI clarification after the literature review is complete
...
**Trigger Conditions**:
- agente_bibliografia finds < 5 usable sources after standard search strategy
- After excluding quality-unqualified sources, < 3 remain

**User Notification Message**:
> With the current search strategy, I found only limited relevant literature. This could mean: (1) this is a very new research area; (2) the search keywords need adjustment; (3) the Pregunta de Investigación scope may need refinement. Let me try expanding the search strategy.
...
5. If still insufficient → suggest the user consider adjusting the PI or accept this as an exploratory study
...
- Adjust PI → return to Phase 1

---

### F3: Methodology Mismatch

**Affected Modes**: `full` (punto de control 1)
**Severity**: High

**Trigger Conditions**:
- devils_advocate_agent at punto de control 1 determines that the methodology proposed by research_architect_agent cannot answer the PI produced by research_question_agent
- There is a logical gap between the methodology and the RQ

**User Notification Message**:
> Abogado del Diablo found an important issue in the methodology review: your Pregunta de Investigación asks "why," but your method design can only answer "whether." Let's go back and adjust — here are three possible directions...

**Handling Steps**:
1. Clearly state the gap between the PI type (descriptive/comparative/causal/evaluative) and the method's capability
2. Provide 3 alternative method suggestions, each with pros and cons
3. Confirm whether the PI needs adjustment to match a feasible method
4. Re-execute research_architect_agent

**Recovery Paths**:
- Select an alternative method → regenerate Methodology Blueprint → punto de control 1 re-review
- Adjust PI → return to research_question_agent → redo Phase 1
- Maximum 2 retries; if still mismatched on the 3rd attempt → suggest the user consult their advisor

---

### F4: Abogado del Diablo CRITICAL

**Affected Modes**: `full` (any punto de control)
**Severity**: Critical

**Trigger Conditions**:
- devils_advocate_agent finds a Critical severity issue at any punto de control
- Includes: fatal logical flaws, core assumptions that cannot hold, evidence contradicting conclusions

**User Notification Message**:
> STOP — Abogado del Diablo found a critical issue that debe ser resolved before continuing:
...
4. After user correction → re-execute the punto de control
5. 2 consecutive CRITICALs → suggest the user fundamentally rethink the research direction

**Recovery Paths**:
- User corrects the issue → re-execute punto de control → continue after PASS
...
**Handling Steps**:
1. Save all outputs from the current stage (PI Brief, INSIGHTs, Bibliography, etc.)
...
- User needs international publication → suggest adjusting PI to add a comparative perspective
...
**Affected Modes**: `full` (Phase 5, punto de control 3)
**Severity**: High
...
3. Re-execute punto de control 3
...
**Trigger Conditions**:
- agente_sintesis attempts interdisciplinary integration but cannot find meaningful connections
