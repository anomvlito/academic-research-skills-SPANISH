...
Comprehensive reference for setting up post-research literature monitoring across major academic databases and platforms. Used by the `agente_monitoreo` to configure monitoring strategies tailored to the user's research field and publication velocity.
...
## HABILIDAD.md Extracted Content: Literature Monitoring (Optional Post-Pipeline)

After any research mode is complete, users can optionally activate the `agente_monitoreo` to set up post-research literature monitoring. This is not part of the main pipeline — it is an auxiliary capability triggered on demand.

See `agents/agente_monitoreo.md` for the detailed agent definition.
