...
investigacion-profunda provides 7 modes suited to different research stages and needs. This guide helps users select the most appropriate mode.
...
User Input
    │
    ├── Have a clear Pregunta de Investigación?
    ...
    │              ├── Yes ──→ socratic mode
    │              └── No ───→ full mode
    │                          (Phase 1 interactive PI clarification)
    │
...
| **Expected Dialogue Rounds** | 2-5 rounds (Phase 1 interaction + punto de controls) |
...
| **Agents Activated** | 4 (PI + Biblio + Verification + Report) |
...
| **Agents Activated** | 3 (Editor + Abogado del Diablo + Ethics) |
...
| **Not Applicable** | Already have a clear Pregunta de Investigación and methodology; need quick report output; only need literature or fact-checking |
...
| **Expected Output** | Research Plan Summary with extracted INSIGHTs, Pregunta de Investigación direction, methodology suggestions |
...
| **Agents Activated** | 8-10 (PI + Architect + Biblio + Verification + RoB + Meta-Analysis/Synthesis + Report + Editor + Ethics) |
...
socratic → articulo-academico    Write paper directly after Socratic completion
...
systematic-review → articulo-academico  Write up systematic review as a paper
```

### investigacion-profunda to articulo-academico Mode Mapping

| investigacion-profunda Mode | Output | Maps to articulo-academico Mode | Description |
|-------------------|------|--------------------------|------|
...
### investigacion-profunda vs articulo-academico-reviewer Mode Mapping

| investigacion-profunda `review` mode | articulo-academico-reviewer |
|------------------------------|------------------------|
...
```
Step 1: investigacion-profunda (socratic/full)
          ↓ Research Plan / Full Report
Step 2: articulo-academico (plan/full)
          ↓ Paper draft
Step 3: articulo-academico-reviewer (full/guided)
          ↓ Review comments
Step 4: articulo-academico (revision)
          ↓ Revised paper
...
- **Reusable**: PI Brief (as-is), initial keyword list
...
- **When**: Socratic dialogue produces a well-formed PI and user wants autonomous research
- **Reusable**: PI Brief (with socratic_insights), accumulated INSIGHTs, scope definition
- **Must Redo**: Everything after PI formulation (bibliography, synthesis, verification, report)
- **Quality Delta**: socratic mode only produces PI Brief; full mode executes the complete pipeline
...
- **Must Redo**: PI formulation (reframe from verification to inquiry), full bibliography, synthesis
...
### Transition: systematic-review → articulo-academico
