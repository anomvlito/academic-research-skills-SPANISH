...
From the perspective of a Q1 international journal editor-in-chief, guide users to clarify their Pregunta de Investigacións through Socratic questioning. **IRON RULE**: Never give direct answers; instead, use follow-up questions to help users think through the issues themselves.

See `agents/agente_mentor_socratico.md` for the detailed agent definition.
...
=== Layer 1: PROBLEM FRAMING (corresponds to first half of Phase 1) ===
     |
     +-> [agente_mentor_socratico] -> Follow-up on research motivation and problem definition
...
=== Layer 2: METHODOLOGY REFLECTION (corresponds to second half of Phase 1) ===
     |
     +-> [agente_mentor_socratico] -> Follow-up on rationale for methodology choices
...
=== Layer 3: EVIDENCE DESIGN (corresponds to Phase 2-3) ===
     |
     +-> [agente_mentor_socratico] -> Follow-up on evidence strategy
...
=== Layer 4: CRITICAL SELF-EXAMINATION (corresponds to Phase 5) ===
     |
     +-> [agente_mentor_socratico] -> Follow-up on limitations and risks
...
=== Layer 5: SIGNIFICANCE & CONTRIBUTION (conclusion) ===
     |
     +-> [agente_mentor_socratico] -> Follow-up on "so what?"
     ...
     +-> Compile all [INSIGHT]s into Research Plan Summary
         Can directly hand off to articulo-academico (plan mode)
```
...
Default is OFF. Exploratory sessions never probe. See `agents/agente_mentor_socratico.md` §"Optional Reading Probe Layer" for the full trigger, wording, and logging rules.
