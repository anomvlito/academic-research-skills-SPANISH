# Traspaso Example: investigacion-profunda → articulo-academico

This example demonstrates how investigacion-profunda full mode, after completing research, hands off to articulo-academico to begin paper writing.

---

## Scenario Setup

The user has completed investigacion-profunda full mode on the topic "AI-Assisted Quality Assurance in Higher Education: A Comparative Analysis of Implementation Strategies in East Asian Universities." Below is a summary of the research outputs.

---

## investigacion-profunda Output Summary

### 1. Pregunta de Investigación Brief (from research_question_agent)

```markdown
### Primary Pregunta de Investigación
...
### 3. Annotated Bibliography (from agente_bibliografia)
...
### 4. Synthesis Report (from agente_sintesis)
...
## Traspaso Instruction

After completing investigacion-profunda, the user inputs:
...
## articulo-academico agente_admision Auto-Detection

**agente_admision Detection Process**:

```
Scanning available materials...

[DETECTED] Pregunta de Investigación Brief
  - Source: investigacion-profunda / research_question_agent
  - Status: Complete (FINER 4.6/5)
  - Action: SKIP articulo-academico Phase 1 (topic scoping)

[DETECTED] Methodology Blueprint
  - Source: investigacion-profunda / research_architect_agent
  - Status: Complete (pragmatist QCA + document analysis)
  - Action: SKIP articulo-academico Phase 2 (methodology design)
  - Note: Will integrate into Method section directly

[DETECTED] Annotated Bibliography (18 sources, APA 7.0)
  - Source: investigacion-profunda / agente_bibliografia
  - Status: Complete, quality-screened
  - Action: SKIP articulo-academico literature search
  - Note: Will use as foundation for Literature Review section

[DETECTED] Synthesis Report
  - Source: investigacion-profunda / agente_sintesis
  - Status: Complete (3 core findings + 1 contradiction + 2 gaps)
  - Action: SKIP articulo-academico analysis phase
  - Note: Will structure Findings and Discussion sections around synthesis

Materials Assessment: COMPREHENSIVE
Recommended articulo-academico modo: completo (with accelerated pipeline)
...
| articulo-academico Step | Normal Workflow | After Traspaso | Reason |
|---------------------|---------|-----------|------|
| Phase 1: Topic Scoping | agente_admision clarifies from scratch | SKIPPED | PI Brief is complete |
...
## Post-Traspaso articulo-academico Actual Workflow

```
=== articulo-academico: Accelerated Pipeline ===

Step 1: STRUCTURAL MAPPING
  [outline_agent]
  - Input: PI Brief + Methodology Blueprint + Synthesis Report
  - Output: Complete paper outline, each section tagged with corresponding investigacion-profunda materials
  - Output example:
...
Step 3: DISCUSSION + BOOKENDS
  [discussion_writer_agent] → Discussion
    ...
    - Integrates investigacion-profunda Knowledge Gaps as Future Research

  [bookend_agent] → Introduction + Conclusion
    - Introduction: from macro context to specific RQ
    - Conclusion: no new information, echoes PI and looks to the future
...
Step 5: SELF-REVIEW
  [review_agent]
    - Internal consistency check (PI → Method → Findings → Discussion logic chain)
...
## User-Facing Traspaso Experience

**articulo-academico's Opening Message**:

> I've read your complete outputs from the investigacion-profunda stage, including:
> - Pregunta de Investigación Brief (FINER 4.6/5)
...
1. **Not copy-paste**: articulo-academico does not directly copy investigacion-profunda outputs, but transforms them into the tone and format of an academic paper
2. **May discover new issues**: During the writing process, articulo-academico agents may discover points missed by investigacion-profunda and will proactively supplement them
...
4. **Review recommendation auto-connects**: After paper completion, the user can continue with `articulo-academico-reviewer` for formal review
