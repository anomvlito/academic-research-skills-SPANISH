...
Este documento is used by `agente_analista_campo` and `agente_editor_jefe` as a reference for calibrating EIC identity and assessing journal fit.
...
- This list cannot be exhaustive; `agente_analista_campo` can recommend more precisely matched journals based on the specific paper topic
