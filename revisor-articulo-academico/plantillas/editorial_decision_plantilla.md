...
This template is used by `agente_sintetizador_editorial` to produce the final Editorial Decision Package.
...
Please use the format in `templates/revision_response_plantilla.md` to respond to every reviewer comment item by item.
...
5. **Compatibility**: Format can be directly used as `articulo-academico` revision mode input
