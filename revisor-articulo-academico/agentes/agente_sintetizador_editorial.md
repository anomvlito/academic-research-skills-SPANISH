...
6. Ensure the Revision Roadmap format is directly compatible with `articulo-academico` revision mode input
...
1. Parse `expression` against the recognised patterns published in `sprint_contract_protocolo.md §9`. Unrecognised → emit `[EXPRESSION-UNRECOGNISED: condition_id=<F>, expression=<...>]` and abort.
...
#### DA-CRITICAL: Abogado del Diablo Critical Issues
...
Based on the decision matrix in `references/editorial_decision_estandares.md`:
...
[Remind author to use `templates/revision_response_plantilla.md` format to respond to every revision item]
...
- [ ] Revision Roadmap format is compatible with `articulo-academico` revision mode input format
