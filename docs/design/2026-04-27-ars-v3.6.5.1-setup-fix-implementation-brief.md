...
The repo has exactly **four skill folders** at the root, each with its own `HABILIDAD.md`:
...
There is **no root `HABILIDAD.md`**. There is no plan to create one. Any install method that requires a single `HABILIDAD.md` at the install path must install each skill folder separately, not the whole repo.
...
```
<install-root>/<skill-name>/HABILIDAD.md
```

For Claude Code project skills: `<project>/.claude/skills/<skill-name>/HABILIDAD.md`.
For Cowork: `~/.claude/skills/<skill-name>/HABILIDAD.md`.
For claude.ai upload: each skill is uploaded as one zip, where the zip's top-level entry is `<skill-name>/HABILIDAD.md` (not `<skill-name>/<skill-name>/HABILIDAD.md`).

Any install instruction that produces `<install-root>/academic-research-skills/<skill-name>/HABILIDAD.md` (extra nesting level) is broken — Cowork / Claude Code / claude.ai will not discover the skill.
...
2. **Issue #44 reply (already vetted UI wording, inlined below for self-contained reference):**
   ...
   > cd ~/.claude/skills
   > ln -s ~/academic-research-skills/investigacion-profunda investigacion-profunda
   > ln -s ~/academic-research-skills/articulo-academico articulo-academico
   > ln -s ~/academic-research-skills/articulo-academico-reviewer articulo-academico-reviewer
   > ln -s ~/academic-research-skills/pipeline-academico pipeline-academico
   > ```
...
- Any `HABILIDAD.md` (description, frontmatter, body) — those are v3.6.5.2 scope
- Any agent file under `*/agentes/`
...
- `INICIO_RAPIDO.md` install-related sections (sweep, only if drifted)
- `HISTORIAL_DE_CAMBIOS.md` (add v3.6.5.1 entry)
...
- Whether HABILIDAD.md description trim is in this patch — already decided NO (v3.6.5.2)
...
```
.claude/skills/academic-research-skills/investigacion-profunda/HABILIDAD.md   ← buried, not discovered
```

instead of the required:

```
.claude/skills/investigacion-profunda/HABILIDAD.md   ← Cowork finds this
```
...
The four skill folder names are exact: `investigacion-profunda`, `articulo-academico`, `articulo-academico-reviewer`, `pipeline-academico`. Use bash code blocks with explicit per-skill commands.
...
- **For Method 4a (Skill upload)**: no GitHub auth needed; zip each skill folder locally so the zip's top-level entry is `<skill-name>/HABILIDAD.md`; upload via Settings → Capabilities → Skills. Note that zip structure errors and the 200-char description cap will surface as upload-time errors per the Custom Skills doc.
...
- Add a `scripts/` row to the directory table with rationale "literature_corpus[] adapters (folder_scan, zotero, obsidian) + schema validators; required for Pasaporte de Materiales corpus mode and CI-style validation."
...
grep -rn -E "Cowork|claude\.ai|claude\.com/product/cowork|GitHub integration|\.claude/skills" \
  README.md README.zh-TW.md INICIO_RAPIDO.md .claude/CLAUDE.md docs/ \
  --exclude-dir=design
```
...
- `INICIO_RAPIDO.md` — if it exists and mentions Cowork or claude.ai
- `.claude/CLAUDE.md` — Descripción General de Habilidades table is unlikely to need changes (no install instructions there), but check
- `docs/SETUP.zh-TW.md` — mirror EN changes if zh-TW exists (may or may not — check first)

Files NOT to touch in v3.6.5.1: anything under `*/agentes/`, `shared/`, `scripts/`, `tests/`, individual `HABILIDAD.md` files.
...
1. **Method 1 (project skills)**: confirm `<project>/.claude/skills/investigacion-profunda/HABILIDAD.md` exists at the expected path and is the file from the repo (not a placeholder). Repeat for the other three skills.
2. **Method 3 Option A (symlink)**: confirm `~/.claude/skills/investigacion-profunda/HABILIDAD.md` resolves (test with `test -f`, not `ls -L`, to follow symlinks). Confirm `readlink` shows the absolute path back to the cloned repo. Repeat for all four skills.
3. **Method 3 Option B (copy)**: confirm `~/.claude/skills/investigacion-profunda/HABILIDAD.md` exists, is a regular file (not symlink), and content matches `<repo>/investigacion-profunda/HABILIDAD.md` (e.g. `diff` returns 0). Repeat for all four skills.
