...
- Entry point: Etapa 3 (REVIEW)
- User type: Experienced researcher

Detected that you have a complete paper — you can proceed directly to Etapa 3 (REVIEW).
...
If you later want to revise based on review comments, I'll automatically guide you to Etapa 4.
```
...
Got it. Pipeline configuration:
  ...
  Etapa 1 RESEARCH    [--] Skipped (you already have a research foundation)
  Etapa 2 WRITE       [--] Skipped (you already have a complete paper)
  Etapa 3 REVIEW:     quick -- EIC quick assessment first, checking the big picture
  Etapa 4 REVISE:     TBD -- based on Etapa 3 results
  Etapa 5 FINALIZE:   TBD -- when needed

Start Etapa 3 (REVIEW) quick mode?
...
```
Entering Etapa 3 (REVIEW) -- using articulo-academico-reviewer quick mode
...
agente_editor_jefe executing quick assessment...
```
...
```
Etapa 3 (REVIEW) quick mode Complete!
...
This is the quick review result. You have several options:
  ...
  a) Proceed directly to Etapa 4 (REVISE) to address these 3 issues
...
OK, updating pipeline:
  ...
  Etapa 3 REVIEW (quick):  [v] Completed
  Etapa 4 REVISE:          [..] Next -- fix C1-C3
  Etapa 3' RE-REVIEW:      full mode -- full review after revision
  Etapa 5 FINALIZE:        TBD

Start Etapa 4 (REVISE)?
...
Next step is Etapa 3' (RE-REVIEW) -- this time using full mode.
...
```
Entering Etapa 3' (RE-REVIEW) -- full mode, Loop 1/2

Loading articulo-academico-reviewer HABILIDAD.md (full mode)...
Passing Revised Draft + Response to Reviewers...
agente_analista_campo re-analyzing...
```
...
These are all minor changes that can be handled during Etapa 5 (FINALIZE).
Or would you prefer a formal Etapa 4 revision before another review?

My recommendation: Since all items are minor, proceed directly to Etapa 5 and handle them together.
...
```
Entering Etapa 5 (FINALIZE) -- format-convert mode
...
agente_formateador executing LaTeX conversion...
...
+-----------------------------------------+
  ...
  Etapa 1 RESEARCH    [--] Skipped
  Etapa 2 WRITE       [--] Skipped
