...
- Goal: Full workflow
- Entry point: Etapa 1 (RESEARCH)
- User preference: First time writing a paper --> recommend guided mode

Recommended Pipeline Configuration:

  Etapa 1 RESEARCH: socratic -- First time doing research, Socratic guidance to help you think through
  Etapa 2 WRITE:    plan     -- Chapter-by-chapter guidance to plan your paper structure
  Etapa 3 REVIEW:   full     -- Complete 4-person review, simulating real submission experience
  Etapa 4 REVISE:   revision -- Item-by-item revision based on review comments
  Etapa 5 FINALIZE: format   -- Convert to your desired format (LaTeX/DOCX via Pandoc/PDF)
...
```
Entering Etapa 1 (RESEARCH) -- using investigacion-profunda socratic mode

Loading investigacion-profunda HABILIDAD.md...
Launching agente_mentor_socratico...
```

**Mentor Socrático**: You mentioned "the impact of declining birthrate on private university enrollment strategies." Before we begin, I'd like to understand a few things:
...
**Mentor Socrático**: Great observation. You mentioned three types of strategies: outreach activities, admission standard adjustments, and new program establishment.

Let me follow up: What do you mean by "actually work"? Higher freshmen enrollment rate? Maintaining student quality? The school surviving? Different definitions of "working" lead to completely different Pregunta de Investigacións.
...
**Mentor Socrático -- Final Summary**:
...
Then agente_bibliografia executes literature search, source_verification_agent verifies sources...

**Etapa 1 Entregables:**
- PI Brief (with FINER scoring)
...
### Orchestrator: Etapa 1 --> 2 Transition

```
Etapa 1 (RESEARCH) Complete!

Entregables:
- PI Brief: Focused on three enrollment strategies' impact on enrollment rates at private universities
...
```
Entering Etapa 2 (WRITE) -- using articulo-academico plan mode

Loading articulo-academico HABILIDAD.md...
Traspaso materials: PI Brief + Bibliography + Synthesis
agente_admision auto-detected investigacion-profunda output, skipping redundant Phase 0 questions
Launching agente_mentor_socratico (plan mode)...
```

**Plan Mode Mentor**: I've reviewed your Research Plan Summary. The Pregunta de Investigación is clear. Let's plan the paper chapter by chapter. Starting with the Introducción.
...
articulo-academico agente_redactor_borrador writing based on Chapter Plan...
agente_cumplimiento_citas verifying citations...
agente_resumen_bilingue writing bilingual abstract...
```
...
### Orchestrator: Etapa 2 --> 3 Transition

```
Etapa 2 (WRITE) Complete!
...
```
Entering Etapa 3 (REVIEW) -- using articulo-academico-reviewer full mode

Loading articulo-academico-reviewer HABILIDAD.md...
Passing Paper Draft to reviewer...
agente_analista_campo analyzing paper domain...
```
...
*(4 reviewers review in parallel... agente_sintetizador_editorial compiles...)*
...
### Orchestrator: Etapa 3 --> 4 Transition

```
Etapa 3 (REVIEW) Complete!
...
Next step is Etapa 4 (REVISE).
I'll pass the Revision Roadmap to articulo-academico revision mode,
guiding you through item-by-item revision.

After revision, we'll enter Etapa 3' (RE-REVIEW) to confirm quality.
...
```
Entering Etapa 4 (REVISE) -- using articulo-academico revision mode

Loading articulo-academico HABILIDAD.md (revision mode)...
Passing Paper Draft + Revision Roadmap...
agente_revisor_pares monitoring revision quality...
```
...
### Orchestrator: Etapa 4 --> 3' Transition

```
Etapa 4 (REVISE) Complete! Revision round: Round 1
...
Next step is Etapa 3' (RE-REVIEW).
...
```
Entering Etapa 3' (RE-REVIEW) -- Loop 1/2
...
### Orchestrator: Etapa 3' --> 5 Transition

```
Etapa 3' (RE-REVIEW) Complete!
...
Since this is Minor Revision (not Major), we can proceed directly to Etapa 5 (FINALIZE).
...
```
Entering Etapa 5 (FINALIZE) -- using articulo-academico format-convert mode

Loading articulo-academico HABILIDAD.md (format-convert mode)...
Simultaneously handling 2 minor revision items...
agente_formateador executing format conversion...
```
...
+-----------------------------------------+
  ...
  Etapa 1 RESEARCH    [v] Completed
    Mode: socratic
    Outputs: PI Brief, Metodología,
             Bibliography (22 sources),
             Synthesis

  Etapa 2 WRITE       [v] Completed
    Mode: plan -> full
    Outputs: Paper Draft
             (5,200 words, IMRaD)

  Etapa 3 REVIEW      [v] Completed
    Mode: full
    Decision: Major Revision
    Required Revisions: 5 items

  Etapa 4 REVISE      [v] Completed
    Revision Round: 1
    Addressed: 5/5 required revisions

  Etapa 3' RE-REVIEW  [v] Completed
    Loop: 1/2
    Decision: Minor Revision (Accept)

  Etapa 5 FINALIZE    [v] Completed
...
- Revise again --> just say "revise the paper" (triggers articulo-academico revision mode)
