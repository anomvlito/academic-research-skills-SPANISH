...
Defines how `agente_orquestador_pipeline` converts FULL punto de controls into reset boundaries when `ARS_PASSPORT_RESET=1` is set. This is the authoritative protocol; any divergent behavior in agent prompts is a bug.

## When this protocol applies

| Flag state | Mode | Behavior at FULL punto de control |
|------------|------|-----------------------------|
| `ARS_PASSPORT_RESET` unset / `=0` | any | Continuation (pre-v3.6.3 default). No reset tag emitted. |
| `ARS_PASSPORT_RESET=1` | `systematic-review` | **Mandatory reset** at every FULL punto de control. |
| `ARS_PASSPORT_RESET=1` | any other mode | **Strong-default reset** at every FULL punto de control. User `continue` response overrides back to continuation for the next stage only. |

MANDATORY punto de controls (integrity Etapa 2.5 / 4.5, review decisions, Etapa 5 finalization) are orthogonal: reset can co-occur with MANDATORY. SLIM punto de controls never trigger reset.

## El protocolo de límite de reinicio

When the orchestrator reaches a FULL punto de control with the flag ON:
...
3. **Emit reset tag.** In the punto de control notification block, append a machine-stable line:
   ```
   [REINICIO-PASAPORTE: hash=<hash>, etapa=<completed>, siguiente=<next>]
   ```
4. **Emit human instruction.** In the same punto de control notification, include a `### Resume Instruction` subsection with:
...
Pasaporte de Materiales ledger (`compliance_history[]` + new `reset_boundary` entries) is append-only:
- Every punto de control with the flag ON appends one `reset_boundary` entry of kind `boundary` under Schema 9's `reset_boundary` field.
- Re-running a stage (e.g., after a review rejection) appends a new entry with `version_label` bumped (`v1.0 → v1.1-revised`).
- `resume_desde_pasaporte` consumption appends one entry of kind `resume` to the same ledger, carrying the `consumes_hash` pointer to the `boundary` entry it resolves. This is how resume leaves a trace — no mutation of prior entries.
...
3. Reset tag is the sole machine-stable Traspaso. Human-readable `### Resume Instruction` is for user ergonomics; consumers parse the tag.
4. `systematic-review` with flag ON refuses in-session continuation across FULL punto de controls.
5. Hash mismatch on resume is a hard error; orchestrator never proceeds on a guessed or coerced hash.
6. MANDATORY punto de controls are not downgraded by reset; they co-occur.
...
- **Collaboration Depth Observer (v3.5.0):** fires on FULL/SLIM as before. Observer output is included in the punto de control notification regardless of reset state. Observer state does NOT carry across resets; each fresh session observes only its own stage.
...
- Does not define Zotero / Obsidian / folder-scan adapter shapes (defined in [`pipeline-academico/referencias/adapters/overview.md`](adapters/overview.md) from v3.6.4+).
...
- [`shared/Traspaso_schemas.md`](../../shared/Traspaso_schemas.md) — Schema 9 definition
- [`pipeline-academico/agentes/agente_orquestador_pipeline.md`](../agentes/agente_orquestador_pipeline.md) — orchestrator integration
- [`pipeline-academico/referencias/pipeline_state_machine.md`](pipeline_state_machine.md) — state transitions
