...
**Applies to**: any agent in this repo that reads `literature_corpus[]` from a Pasaporte de Materiales
...
This is the contract every literature-reading consumer agent must follow. The v3.6.4 input port (`shared/contracts/passport/literature_corpus_entry.schema.json`) defines what enters a Pasaporte de Materiales; this document defines how Phase 1 agents read it.
...
Consumer agents never modify, backfill, or derive new content into `literature_corpus[]`. Read only. The Pasaporte de Materiales's corpus is the user's curated list; any rewriting belongs to the user's adapter (v3.6.4 input port), not Phase 1 prompt-layer agents.

### Iron Rule 4 — Graceful fallback on parse failure

Consumer agents do NOT re-validate schema, do NOT parse JSON Schema at runtime, and do NOT dereference `source_pointer` URIs. The v3.6.4 input-port lint validates adapter output, but a passport may reach a Phase 1 agent through other paths (hand-edits, `resume_desde_pasaporte`, assembled passports). When a consumer cannot parse `literature_corpus[]`, emit `[CORPUS PARSE FAILURE: <cause>]` in the Search Strategy Report and fall back to external-DB-only flow. No abort Phase 1, do not attempt schema repair, do not invent contents.
