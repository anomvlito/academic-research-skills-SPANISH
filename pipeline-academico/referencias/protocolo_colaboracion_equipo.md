...
|------|----------------|-----------------|
| Research Lead | Etapa 1 (RESEARCH) | Defines RQ, manages literature search, approves synthesis, sets theoretical framework |
| Lead Author | Etapa 2 (WRITE), Etapa 4/4' (REVISE) | Writes/revises paper, makes final content decisions, owns the manuscript |
| Methods Specialist | Etapa 1 (methodology), Etapa 2 (methods section) | Ensures methodological rigor, validates statistical analysis, reviews data integrity |
| Review Coordinator | Etapa 3/3' (REVIEW) | Manages simulated review process, distributes feedback, facilitates revision coaching |
| Integration Lead | All stages | Ensures consistency across stages, manages Traspasos, resolves cross-stage conflicts |
...
For each stage transition, the following Traspaso procedure applies:

### Etapa 1 -> Etapa 2 (Research -> Write)

| Item | Detail |
|------|--------|
| **Who hands off** | Research Lead |
| **Who receives** | Lead Author |
| **Materials** | PI Brief, Bibliography, Synthesis Report (conforming to Schemas 1-3 in `shared/Traspaso_schemas.md`) |
| **Approval needed** | Research Lead confirms synthesis is complete and PI is finalized |
| **Traspaso checklist** | All Pasaporte de Materialess (Schema 9) attached; Bibliography minimum source count met; Synthesis has 3+ themes |

### Etapa 2 -> Etapa 2.5 (Write -> Integrity)
...
| **Traspaso checklist** | All sections complete; reference list formatted; word count within target range |

### Etapa 2.5 -> Etapa 3 (Integrity -> Review)
...
| **Traspaso checklist** | Integrity Report attached; all SERIOUS/MEDIUM issues resolved |

### Etapa 3 -> Etapa 4 (Review -> Revise)
...
| **Traspaso checklist** | All revision items categorized and prioritized; coaching session completed (or skipped by Lead Author) |

### Etapa 4 -> Etapa 3' (Revise -> Re-Review)
...
| **Traspaso checklist** | Response to Reviewers covers every roadmap item; new references verified |

### Etapa 4.5 -> Etapa 5 (Final Integrity -> Finalize)
...
| **Traspaso checklist** | All previous integrity issues confirmed resolved; Pasaporte de Materiales updated to VERIFIED |
...
```
main
  |-- paper/draft-v1          (Etapa 2 output)
  |-- paper/post-integrity-v1 (Etapa 2.5 output)
  |-- paper/post-review-v1    (Etapa 4 output)
  |-- paper/final-v1          (Etapa 5 output)
```
...
| Tag | When | Example |
|-----|------|---------|
| `v0.1-draft` | After Etapa 2 completion | First complete draft |
| `v0.2-post-integrity` | After Etapa 2.5 PASS | Draft verified for integrity |
| `v0.3-post-review` | After Etapa 4 completion | First revision complete |
| `v0.4-post-rereview` | After Etapa 4' completion (if applicable) | Second revision complete |
| `v1.0-final` | After Etapa 5 completion | Final manuscript |

### Reglas

- Never overwrite; always create a new version
- All versions are preserved for audit trail
- Version labels must match the Pasaporte de Materiales `version_label` field (Schema 9 in `shared/Traspaso_schemas.md`)
...
### 1. Traspaso Notification

```markdown
## Traspaso: Stage [X] -> Stage [Y]
...
**Deadline**: [date]
**Return Format**: [Use Schema 6 format from shared/Traspaso_schemas.md]
```
...
Person A (Research Lead + Lead Author):
  - Runs Etapa 1 (RESEARCH) with their Claude session
  - Runs Etapa 2 (WRITE) with their Claude session
  - Receives revision items, runs Etapa 4/4'
  - Runs Etapa 5 (FINALIZE)

Person B (Methods Specialist + Review Coordinator):
  - Reviews Etapa 1 methodology output
  - Reviews Etapa 2 methods section
  - Manages Etapa 3/3' review process
  - Handles specific methodology revision items

Traspaso: via shared folder or git repo
Materials: conform to schemas in shared/Traspaso_schemas.md
```
...
Add Integration Lead role:
  ...
  - Validates Traspaso completeness
...
- Traspaso materials (conforming to schemas in `shared/Traspaso_schemas.md`) ensure consistency across sessions
