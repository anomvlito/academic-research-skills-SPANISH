...
agents_used:
  - agente_formateador
  - agente_cumplimiento_citas
  - agente_revisor_pares
...
The user has a completed paper titled "The Impact of Micro-Credential Certification on Employability of Vocational Education Students in Taiwan," which has received peer review comments from a journal (3 major + 4 minor). The user employs articulo-academico's revision mode for systematic revision. This example demonstrates the complete workflow from comment parsing, citation correction, review verification, to the Response to Reviewers letter.
...
## agente_formateador Parses Revision Comments
...
The agente_formateador recommends the following revision order:
...
## agente_cumplimiento_citas Corrects Citation Issues
...
In conjunction with M2's literature update requirement, the agente_cumplimiento_citas verifies format compliance of newly added references:
...
## agente_revisor_pares Reviews the Revised Version
...
2. Removed 3 pre-2017 references with lower relevance to the Pregunta de Investigacións.
