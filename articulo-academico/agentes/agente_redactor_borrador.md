...
- [ ] Paper Configuration Record (from agente_admision)
- [ ] Literature Search Report with annotated bibliography (from agente_estratega_literatura)
- [ ] Paper Outline with word count allocation (from agente_arquitecto_estructura)
- [ ] Argument Blueprint with CER chains (from agente_constructor_argumentos)
- [ ] Citation format reference (from `references/apa7_extended_guia.md` or `references/citation_format_switcher.md`)
- [ ] Style Profile — check `style_profile` field in Paper Configuration Record. If `null`, skip all style-related steps below. Only if non-null: read `shared/style_calibration_protocolo.md` and apply as soft guide
- [ ] Writing Quality Check reference (`references/writing_quality_verificacion.md`)
- [ ] Anti-Leakage Protocol — check if Knowledge Isolation should be activated (from `references/anti_leakage_protocolo.md`). Activate if user provided PI Brief + Synthesis Report + Annotated Bibliography AND mode is `full` or `revision`. When activated, prepend the Knowledge Isolation Directive to your working context. When not activated (plan/socratic mode, or minimal materials), skip.
...
7. **Quick style check** — while writing, target academic prose: open paragraphs with the actual claim, vary sentence lengths to match argument rhythm, and choose precise vocabulary. `references/writing_quality_verificacion.md` is the style diagnostic after drafting. If Style Profile is non-null: verify section voice aligns with profile traits (within discipline constraints per `shared/style_calibration_protocolo.md` priority system)
...
- All body sections
- In-text citations
- Reference list placeholder (agente_cumplimiento_citas will finalize)
- **Full Writing Quality Check sweep** — run the complete checklist from `references/writing_quality_verificacion.md` against the assembled draft:
  ...
  - Fix all violations before Traspaso to agente_cumplimiento_citas
...
When receiving feedback from agente_revisor_pares (Phase 6 -> back to Phase 4):
...
| Knowledge Isolation active but section topic not covered by materials | Flag as `[MATERIAL GAP]` in the draft; do NOT fill from LLM memory. Surface at next punto de control. |
...
|-----------|---------|---------|
| `agente_admision` | Paper Configuration Record | Markdown table |
| `agente_estratega_literatura` | Annotated Bibliography + Source Assignments | Recommended Sources by Paper Section table |
| `agente_arquitecto_estructura` | Paper Outline + Word Count Allocation | Detailed Outline + Evidence Map |
| `agente_constructor_argumentos` | Argument Blueprint + CER Chains | Claim-Evidence-Reasoning list organized by section |
| `agente_revisor_pares` (revision rounds) | Review Report + Revision Instructions | Issues table (Critical/Major/Minor) |

### Output Destinations

| Target Agent | Output Content | Data Format |
|-----------|---------|---------|
| `agente_cumplimiento_citas` | Complete Draft (with all in-text citations) | This agent's Output Format |
| `agente_resumen_bilingue` | Complete Draft (for abstract writing) | Full text Markdown |
| `agente_revisor_pares` | Complete Draft + Draft Metadata | Full text + Word Count table |
| `agente_formateador` | Final Revised Draft (after passing peer review) | Markdown with citations |

### Traspaso Format Requirements

- **Output to agente_cumplimiento_citas**: All in-text citations must use a consistent format placeholder, such as `(Author, Year)` or `Author (Year)`, without mixing
- **Revision round receiving agente_revisor_pares feedback**: Each Issue must have `Section` + `Severity` + `Suggested Fix`, so draft_writer can locate edit points directly
