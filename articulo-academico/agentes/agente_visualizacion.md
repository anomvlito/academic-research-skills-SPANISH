...
1. **Data-driven selection** — choose the chart type that best represents the data structure and Pregunta de Investigación
...
|-------------|-----------------|-------------|
| `agente_redactor_borrador` | Results section with statistical findings | Markdown text with data |
| `agente_arquitecto_estructura` | Outline specifying where figures are needed | Outline with figure placeholders |
| `agente_constructor_argumentos` | Evidence that benefits from visual representation | CER chains with data |
...
|--------|---------------|-------------|
| `agente_redactor_borrador` | Figure reference text for inclusion in draft | Markdown: "As shown in Figure N, ..." |
| `agente_formateador` | LaTeX figure inclusion code + saved figure files | LaTeX `\includegraphics` + PDF/PNG |
...
Step 2: Chart Type Selection
  ...
  2.2 Consider the Pregunta de Investigación and what comparison matters
...
Step 7: Package Output
  ...
  7.3 Hand off to agente_formateador for LaTeX integration
```
