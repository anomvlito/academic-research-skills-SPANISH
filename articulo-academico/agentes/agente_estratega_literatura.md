...
| Relevance | Directly addresses PI | Tangentially related |
...
## Reading `literature_corpus[]` from Pasaporte de Materiales (v3.6.5+)

**Backpointer**: see [`pipeline-academico/referencias/literature_corpus_consumers.md`](../../pipeline-academico/referencias/literature_corpus_consumers.md) for the full consumer protocol, BAD/GOOD examples, and shared template.

When the input Pasaporte de Materiales carries a non-empty `literature_corpus[]`, this agent enters the **corpus-first, search-fills-gap** flow. The flow has five steps and four Iron Rules; the PRE-SCREENED block makes corpus utilisation reproducible. The merged `final_included` set feeds the Annotated Bibliography, Literature Matrix, Research Gap Identification, and Recommended Sources by Paper Section sections above without altering their formats.
...
```
derive uncovered_topics = PI subtopics − {topics covered by pre_screened_included[]}
...
- Zero-hit note (emit per F3 only when Included: 0):
  ...
  causes are (a) corpus is stale relative to current RQ, (b) PI has
...
- Zero-hit note (corpus non-empty, 0 included after screening): possible causes
  are (a) corpus is stale relative to current RQ, (b) PI has shifted away from
...
| Relevance to PI | Directly addresses core question | Addresses partial aspects | Provides background only |
...
| Citation format | Per selected format (APA/Chicago/...) | Chinese APA format (see `apa7_chinese_citation_guia.md`) |
...
|--------|---------|
| PI not clearly defined | Return to agente_admision for user to clarify -> cannot start search |
...
### Poor Quality Upstream (agente_admision output is poor)

- If Paper Configuration Record's PI is vague -> infer 2-3 possible search directions from RQ, list for user to choose
...
|-----------|---------|---------|
| `agente_admision` | Paper Configuration Record | Markdown table (with RQ, discipline, language, year range) |
| `investigacion-profunda` (Traspaso) | Annotated Bibliography | APA 7.0 format annotated bibliography |

### Output Destinations

| Target Agent | Output Content | Data Format |
|-----------|---------|---------|
| `agente_arquitecto_estructura` | Literature Search Report (with literature matrix + research gaps) | Markdown (this agent's Output Format) |
| `agente_constructor_argumentos` | Sources categorized by theme + stance tags per source | Literature Matrix |
| `agente_redactor_borrador` | Annotated Bibliography (sources assigned by section) | Recommended Sources by Paper Section table |
| `agente_cumplimiento_citas` | Complete reference information (authors, year, DOI) | Bibliographic information from annotated bibliography |

### Traspaso Format Requirements

- **Output to agente_arquitecto_estructura**: Literature Matrix must include `Quality` field (High/Medium/Low) so architecture agent can prioritize assigning high-quality sources to core sections
- **Output to agente_constructor_argumentos**: Each source annotation must tag whether the source "supports", "opposes", or is "neutral" in viewpoint
- **Traspaso receiving rules**: Bibliography received from investigacion-profunda goes directly to Phase B (full-text assessment), skipping Phase A
