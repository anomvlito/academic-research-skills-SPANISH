...
**Status**: v3.2
**Parent skill**: `articulo-academico`
...
3. **What ARS did**: the mode reads the paper's commit history / pipeline log (if using the full `pipeline-academico`) to identify which AI-assisted steps produced which parts of the paper. At minimum: research assistance, drafting assistance, revision assistance, citation checking, peer review simulation. If the pipeline log is not available, ask the user to confirm which categories apply.
...
| Peer review simulation | `articulo-academico-reviewer` was used on the draft pre-submission |
...
## Integration with existing journal_submission_guia.md

`journal_submission_guia.md` retains the two generic templates (Minimal / Detailed) as fallback for venues not in the v1 database. Disclosure mode's output supersedes those templates when the venue is known. The guide is updated to point to this mode for known venues.
...
- `journal_submission_guia.md` — existing generic templates (fallback)
- `credit_authorship_guia.md` — existing CRediT authorship best practices
