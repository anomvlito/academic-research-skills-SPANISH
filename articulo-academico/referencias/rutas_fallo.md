...
Este documento records the failure scenarios that the articulo-academico skill may encounter at each stage, their trigger conditions, and handling strategies. All agents should refer to this guide when they detect a failure scenario.
...
|---|---------|---------|--------|---------|
| F1 | Insufficient research foundation | Plan mode Step 0 finds no PI / no data | High | Recommend running `investigacion-profunda` first |
...
| F9 | Incomplete Traspaso materials | From investigacion-profunda but missing key materials | Low | List missing items, suggest supplementing or re-running |
...
- User cannot describe their Pregunta de Investigación in one sentence
...
3. Recommend using investigacion-profunda (socratic mode)
4. Explain that they can come back to continue after investigacion-profunda is completed
5. If the user insists on continuing, switch to outline-only mode (low risk)
```

**Response Template**:
```
Your research topic is very interesting, but I notice that a clear Pregunta de Investigación
and literature foundation are still missing.

I recommend you first use the investigacion-profunda tool to:
...
**Trigger Timing**: Phase 2 (agente_arquitecto_estructura)

**Detection Indicators**:
- PI is a causal question but a Literature Review structure was selected
...
**Handling Process**:
```
1. Point out the mismatch between PI and structure
...
**Trigger Timing**: Phase 4 (after agente_redactor_borrador completes)
...
**Trigger Timing**: Phase 5a (agente_cumplimiento_citas)
...
**Trigger Timing**: Phase 5b (agente_resumen_bilingue)
...
**Trigger Timing**: Phase 6 (agente_revisor_pares issues Reject)
...
2. Classify the nature of the problems:
   ...
   c. Fundamental issues (PI infeasible, insufficient data) → Return to Phase 0 for re-evaluation
...
### F9: Incomplete Traspaso Materials

**Trigger Timing**: agente_admision detects investigacion-profunda materials but they are incomplete

**Detection Indicators**:
- Has PI but missing Annotated Bibliography
...
3. Recommend:
   a. Return to investigacion-profunda to complete the missing parts
   b. Or supplement within articulo-academico (add Phase 0 interview questions)
```
...
**How to Resume**: Bring this record and restart articulo-academico; can continue from Phase {N}
```
...
```
F1 (Insufficient research foundation) → Recommend investigacion-profunda → May encounter F9 (incomplete materials) upon return
...
| **Formatting Non-Compliance** | Immediate rejection for template/length/style violations | Review target journal's author guidelines; use `agente_formateador` to reformat; resubmit (often same journal accepts after formatting fix) |
| **Poor Opening** | No specific reason given; likely the abstract/introduction failed to hook | Rewrite abstract with the CARS model (Create A Research Space); lead with the gap, not the background; have `agente_revisor_pares` evaluate the new opening |
...
| F2 | structure_architect cross-validates the match between PI and structure |
...
| F9 | agente_admision performs a complete materials check when detecting a Traspaso |
...
| F12 | agente_admision detects whether this is a conference paper expansion; calculate new content ratio early |
