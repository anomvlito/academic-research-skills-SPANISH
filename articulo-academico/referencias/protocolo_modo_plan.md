...
=== Step 0: RESEARCH READINESS CHECK ===
     |
     +-> [agente_mentor_socratico] -> Confirm what materials the user already has
         - "What research materials do you currently have? (literature, data, analysis results)"
         - "Is your Pregunta de Investigación finalized? Can you state it in one sentence?"
         -> If research foundation is lacking, recommend running investigacion-profunda (socratic mode) first
     |
=== Step 1: THESIS CRYSTALLIZATION ===
     |
     +-> [agente_mentor_socratico] -> Probe the core thesis
...
=== Step 2: CHAPTER-BY-CHAPTER NEGOTIATION ===
     ...
     |
     +-> [agente_mentor_socratico] -> Probe the purpose and content of each chapter
     |
     |   Introduction:
     ...
     After each chapter concludes, [agente_mentor_socratico] extracts a Chapter Summary
     |
     +-> [agente_arquitecto_estructura] -> Produce complete outline based on all Chapter Summaries
     |
=== Step 3: ARGUMENT STRESS TEST ===
     |
     +-> [agente_mentor_socratico + agente_constructor_argumentos]
...
-> Or use articulo-academico-reviewer to review the Chapter Plan
```
