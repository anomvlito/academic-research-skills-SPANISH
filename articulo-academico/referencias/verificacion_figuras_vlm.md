...
- **Obligatorio**: When the pipeline is in `final-check` mode (Etapa 4.5+)
...
```
Step 1: agente_visualizacion generates figure code
...
Step 5: If any FAIL:
  ...
  - agente_visualizacion modifies code to fix
...
When VLM verification is run, the Figure Package (from agente_visualizacion) includes:
